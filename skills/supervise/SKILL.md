...
4. **Dispatch.** Independent tasks with disjoint file sets → parallel (one message, multiple Agent calls). Overlapping or dependent → sequential. This is a Perforce project — do not use git worktree isolation. Only one build/editor instance can run at a time (locked DLLs) — serialize anything that compiles.
